/*
  Script for _finite-trace_ BST descent model. Expectations:
...
function wasVisited(n) {  
  return instance.signature('SearchState').tuples().some((st) => {
      const localCurrent = st.join(instance.field('current'))
      return firstAtomOf(n) == firstAtomOf(localCurrent)
  })
}
function isTargetKey(k) {
  return firstAtomOf(Search.join(target)) == firstAtomOf(k)
}
...
stage.add(new TextBox({text: `Target: ${Search.join(target)}`, coords:{x:100, y:25}}))
stage.add(tree)
stage.render(svg)
